/**
 * EVE Nomad Mobile - Logout Service
 *
 * Handles complete logout flow with token cleanup and state reset.
 */

import { apiClient } from '../api/client';
import { removeToken, removeRefreshToken, clearStorage } from '../storage';
 ...
 * 2. Clear JWT token from MMKV
 * 3. Clear refresh token from MMKV
...
export const logout = async (): Promise<void> => {
  try {
    ...
    // Note: clearStorage() will remove ALL data from MMKV
